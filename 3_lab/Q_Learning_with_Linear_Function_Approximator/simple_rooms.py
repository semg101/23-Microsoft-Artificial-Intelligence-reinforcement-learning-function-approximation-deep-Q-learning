...
### Interface  ##An environment must have some representation of the state  of which the agent is interacting with.
class Environment(object):    # In addition, an environment must be able to reset itself and step to the next state.  
    def reset(self):
...
class ActionSpace(object):  #The actions() function maintains the information of how many type of actions in the environment
...
class SimpleRoomsEnv(Environment):   #The SimpleRoomsEnv is a simple environment of a 4x4 rooms, limited by walls
    """Define a simple 4-room environment""" #The initial state has the agent starting at the room on top left corner, with the goal to reach the room at the bottom right corner 
    ...
    def step(self, action):   #the step() function should take in an action and at the minimum, return the next state and the reward()
        s_prev = self.s
    ...
    def reset(self):    #The reset() function should return the initial state
        self.nstep = 0
        self.s = 0
        self.is_reset = True
